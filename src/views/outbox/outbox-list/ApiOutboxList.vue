        ...
        >
          <div class="d-flex align-items-center justify-content-end">
            ...
            <v-select
              ...
              label="name"
              :reduce="name => name.id"
              class="outbox-filter-select mr-1  ml-auto"
    ...
    <template #cell(show_details)="row">
      ...
      <template #cell(sms_count)="data">
        <span class="text-nowrap">
          ...
          variant="info"
          >
          <span v-if="data.item.sms_count===1">
          {{ data.item.sms_count }} Page
          </span>
          <span v-if="data.item.sms_count > 1">
          {{ data.item.sms_count }} Pages
          </span>
          </b-badge>
          <b-tooltip
            title="Sms Lemgth"
            ...
            placement="topright"
          >
          <p class="mb-0" v-if="data.item.sms_count===1">
            Message Length: {{ data.item.sms_count }} page
          </p>
          <p class="mb-0" v-if="data.item.sms_count > 1">
            Message Length: {{ data.item.sms_count }} pages
          </p>
        </b-tooltip>
        </span>
      </template>
      ...
      <template #cell(created_at)="data">
        ...
        >
          {{ formatDateToMonthLong(data.item.created_at) }}
        <b-tooltip
            title="Sent At"
            class="cursor-pointer"
